...
class Faql4Compiler {
  CompiledQuery compile(String source) {
    ...
    final select =
        RegExp(r'\bselect\s+(\w+)\s*,\s*"([^"]*)"').firstMatch(source);
    if (from == null || select == null || from[2] != select[1]) {
      throw Faql4ValidationError(
          'A query needs matching from/select variables.');
    }
    if (!_views.contains(from[1]))
      throw Faql4ValidationError('Unknown view ${from[1]}.');
    ...
    final whereMatch =
        RegExp(r'\bwhere\s+([\s\S]*?)\s*\bselect\b').firstMatch(source);
    final expression = whereMatch == null
        ? null
        : _ExpressionParser(whereMatch[1]!, from[2]!).parse();
    if (mode == FactMode.conservative &&
        expression?.hasUnsafePartialNegation == true) {
      throw Faql4ValidationError(
          'Conservative queries cannot negate partial predicates.');
    }
...
class Faql4Evaluator {
  List<Faql4Violation> evaluate(
    ...
    final view = query.mode == FactMode.conservative
        ? store.conservative
        : store.expanded;
    final hits = <Faql4Violation>[];
    for (final node
        in view.nodes.where((node) => _matchesView(query.view, node, view))) {
      final context =
          _EvaluationContext(store, view, {query.variable: node.id});
      if (query.where == null || query.where!.evaluate(context) == true)
        hits.add(Faql4Violation(node.id, query));
    }
    return hits;
  }
}
...
bool _matchesView(String view, FactNode node, FactStoreView facts) {
  ...
  switch (view) {
    ...
    case 'InteractiveControl':
      return values['tap'] == true || values['longPress'] == true;
    case 'MaterialButtonControl':
      return const {
        'iconButton',
        'elevatedButton',
        'textButton',
    ...
    case 'ImageNode':
      return node.widgetType == 'Image' || node.widgetType == 'CircleAvatar';
    case 'SemanticsNode':
      return node.widgetType == 'Semantics';
    case 'MergeSemanticsNode':
      return node.widgetType == 'MergeSemantics';
    case 'ListTileNode':
      return node.widgetType.endsWith('ListTile');
  }
  return false;
}
...
class _EvaluationContext {
  ...
  Object? property(String variable, String name) {
    ...
    if (name == 'getWidgetType')
      return store.nodes.firstWhere((node) => node.id == id).widgetType;
    for (final fact in view.factsFor(id))
...
class _Predicate extends _Expression {
  ...
  final String name;
  @override
  bool? evaluate(_EvaluationContext c) {
...
class _Not extends _Expression {
  _Not(this.inner);
  final _Expression inner;
  @override
  bool? evaluate(_EvaluationContext c) => !(inner.evaluate(c) ?? false);
  @override
  bool get hasUnsafePartialNegation =>
      inner is _Predicate && (inner as _Predicate).name == 'hasAccessibleLabel';
}
...
class _Binary extends _Expression {
  ...
  @override
  bool get hasUnsafePartialNegation =>
      left.hasUnsafePartialNegation || right.hasUnsafePartialNegation;
}
...
class _ExpressionParser {
  ...
  _Expression parse() {
    ...
    final or = _split(parts, ' or ');
    if (or != null)
      return _Binary(_ExpressionParser(or.$1, variable).parse(), 'or',
          _ExpressionParser(or.$2, variable).parse());
    final and = _split(parts, ' and ');
    if (and != null)
      return _Binary(_ExpressionParser(and.$1, variable).parse(), 'and',
          _ExpressionParser(and.$2, variable).parse());
    if (parts.startsWith('not '))
      return _Not(_ExpressionParser(parts.substring(4), variable).parse());
    final comparison = RegExp(
            r'^(\w+)\.(\w+)\(\)\s*(=|!=)\s*(?:"([^"]*)"|(true|false)|(\d+))$')
        .firstMatch(parts);
    if (comparison != null && comparison[1] == variable) {
      ...
      return _Comparison(
          comparison[1]!, comparison[2]!, comparison[3]!, value!);
    }
    final match = RegExp(r'^(\w+)\.(\w+)\(\)$').firstMatch(parts);
    if (match == null || match[1] != variable)
      throw Faql4ValidationError('Invalid Core expression: $parts');
  ...
  (String, String)? _split(String value, String needle) {
    ...
    return index < 0
        ? null
        : (value.substring(0, index), value.substring(index + needle.length));
  }
}

  ...
  group('SemanticTree grouping heuristics', () {
    test('Row children receive same layoutGroupId', () async {
      ...
      expect(ids.length, equals(1),
          reason: 'All Row children should share a layoutGroupId');
      expect(children.first.layoutGroupId, isNotNull);
    });

    test(
        'Contiguous ListTile children receive listItemGroupId and primary marker',
        () async {
      final tree = await buildTestSemanticTree(
      ...
      expect(third.listItemGroupId, isNull,
          reason: 'Non-list item should not be grouped');
    });
  });
}
